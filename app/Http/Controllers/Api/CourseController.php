...
class CourseController extends Controller
{
    ...
    public function register(CourseRegistrationRequest $request)
    {
        ...
        // build data
        $data = [
            'title' => $course->title.' – '.$course->course_date->format('d.m.Y').($course->course_start ? ' – '.$course->course_start : ''),
            'course_id' => $course->id,
        ...
        try {
            Notification::route('mail', env('MAIL_TO_COURSE'))->notify(new OwnerInformation($data));
        } catch (\Exception $e) {
            \Log::error('Failed to send owner notification for course registration: '.$e->getMessage(), [
                'email' => env('MAIL_TO_COURSE'),
                'course_id' => $course->id,
                'data' => $data,
            ]);
        }
