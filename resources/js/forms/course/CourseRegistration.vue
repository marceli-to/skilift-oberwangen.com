<template>
  <template v-if="!hasOpenSeats">
    ...
    <p class="text-scarlet">Es sind leider keine Plätze mehr verfügbar und eine Anmeldung ist nicht mehr möglich.</p>
  </template>
  <template v-else>
    <h2 class="text-xl md:text-2xl leading-none mb-20 lg:mb-40">
      Anmeldung
    </h2>
...
async function submitForm() {
  ...
  try {
    ...
    const recaptchaToken = await executeRecaptcha('course_registration');
